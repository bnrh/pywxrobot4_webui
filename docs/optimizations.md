...
[db_connection.py](../db_connection.py) 为每个线程缓存 SQLite 连接：
...
- [plugin_log_store.py](../plugin_log_store.py)
- [plugin_base.py](../plugin_base.py) 中的 `PluginStateStore`
...
- 使用 Vite 打包前端资源，并用 git hash 刷新资源版本戳，避免缓存脏读；
...
以下标志写在插件模块顶层，由 [manager](../manager) 读取并下发给控制台，**不必再改** [app_config.py](../app_config.py) 白名单：
